...
def get_closest_value(iter: list, value: int):
    '''
...
def get_quality_url(data: dict,
    ...
    # If key is a string or a 'Quality' constant
        ...
        if q == 'best': key = max(qualities.keys())
        elif q == 'worst': key = min(qualities.keys())
        elif q == 'middle':
            keys = sorted(qualities.keys())
            key = keys[len(keys) // 2]
        else:    
            raise TypeError('Invalid quality type', q)
